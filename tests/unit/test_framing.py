...
class TestNegotiationFraming:
    ...
    def test_decode_struct_error_wrapped_in_framing_error(self):
        """Test that struct.error during decode is wrapped in FramingError."""
        from unittest.mock import patch

        framing = NegotiationFraming()

        # Patch SSLResponse.decode to raise struct.error, exercising the
        # except struct.error branch in NegotiationFraming.try_parse.
        with patch("pygwire.messages.SSLResponse.decode", side_effect=struct.error("boom")):
    ...
    def test_insufficient_data_at_nonzero_offset(self):
        """Test returns None when pos is at the end of the buffer."""
        framing = NegotiationFraming()
        result = framing.try_parse(
            memoryview(b"S"),
            1,
...
class TestLookupFraming:
    ...
    def test_startup_backend_returns_standard_framing(self):
        """Test that STARTUP phase with BACKEND falls through to StandardFraming.

        Only STARTUP+FRONTEND is registered as StartupFraming. The backend
        doesn't receive startup-framed messages, so it gets standard framing.
        """
        framing = lookup_framing(ConnectionPhase.STARTUP, MessageDirection.BACKEND)
        assert isinstance(framing, StandardFraming)
